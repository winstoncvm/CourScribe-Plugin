...
jQuery(document).ready(function ($) {
    ...
    // Add new objective
    $('#addModuleObjectiveBtn').on('click', function () {
        let newObjective = `
            ...
            <div class="objective-item mb-3 p-2">
                <div class="row mb-2">
                    ...
                    <div class="col-md-6">
                        ...
                        <select class="form-control bg-dark text-light action-verb w-100" name="objectives[${objectiveIndex}][action_verb]" id="form-control bg-dark text-light action-verb w-100" id="action-verb-${objectiveIndex}">
        ...
        `;
        $('#module-objectives-container').append(newObjective);
        $(`#thinking-skill-${objectiveIndex}`).trigger('change');
        objectiveIndex++;
    });

    // Delete objective
    $('#module-objectives-container').on('click', '.delete-objective', function () {
        if ($('#module-objectives-container .objective-item').length > 1) {
    ...
    $('#courscribe-module-form').on('submit', function (e) {
        let errors = [];
        if (!$('#module-name').val().trim()) {
            errors.push('Module name is required.');
        }
        if (!$('#module-goal').val().trim()) {
            errors.push('Module goal is required.');
        }
    ...
    $('.add-module').on('click', function () {
        ...
        $('#courscribe-module-form').find('input[name="course_id"]').val(courseId);
        $('#module-name').val('');
        $('#module-goal').val('');
        $('#module-objectives-container').html(`
            <div class="text-dividerr d-flex justify-content-between align-items-center mb-3">
                <span class="divider-textt">Objective Formation</span>
                <img src="${window.location.origin}/wp-content/uploads/2024/12/Rectangle-1501.png" alt="divider" style="width: 32%;">
                <button type="button" class="remove-btn btn-sm delete-objective">Remove</button>
            ...
            <div class="objective-item mb-3 p-2">
                <div class="row mb-2">
                    <div class="col-md-6">
                        <label for="thinking-skill-0" class="form-label text-light">Thinking Skill</label>
                        <select class="form-control bg-dark text-light thinking-skill w-100" name="objectives[0][thinking_skill]" id="thinking-skill-0">
                            <option value="Know">Know</option>
                            <option value="Comprehend">Comprehend</option>
                            <option value="Apply">Apply</option>
                            <option value="Analyze">Analyze</option>
                    ...
                    <div class="col-md-6">
                        <label for="action-verb-0" class="form-label text-light">Action Verb</label>
                        <select class="form-control bg-dark text-light action-verb w-100" name="objectives[0][action_verb]" id="action-verb-0">
                            <option value="" disabled selected>Select Action Verb</option>
                        </select>
                    </div>
                    <div class="col-md-12">
                        <label for="objective-description-0" class="form-label text-light">Objective Description</label>
                        <input type="text" class="form-control bg-dark text-light objective-description w-100" name="objectives[0][description]" id="objective-description-0" placeholder="Enter description" />
                    </div>
                </div>
            </div>
        `);
        objectiveIndex = 1;
        $('#thinking-skill-0').trigger('change');
